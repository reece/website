<template>
  <span class="relative inline-block" :aria-label="text" role="text">
...
/**
 * Animates a single string from its previous value to `text` whenever `text` changes,
 * using the given transition mode. Owns its own display/animation state, so multiple
 * instances can animate independently without interfering with each other. Respects
 * prefers-reduced-motion by swapping text instantly with no animation.
 *
 * @example
 * <TextTransition text="connect with people" mode="fade" />
 */
...
const props = withDefaults(defineProps<{
  /** Target text to display. Animates from the previous value whenever this changes. */
  text: string
  /** Transition mode to use for the next change. */
  mode?: TransitionMode
}>(), {
  mode: 'fade',
})
...
watch(() => props.text, (to, from) => {
  ...
  useLetters.value = LETTER_MODES.has(props.mode)
  runTransition(props.mode, from ?? '', to, () => emit('done'))
})
...
defineExpose({
  /** Current displayed text, for testing/inspection. */
